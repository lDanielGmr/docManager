...
<%
  ...
  if (u==null) { response.sendRedirect("usuario.jsp"); return; }
%>
  <div class="menu-container">
    <div class="menu-box">
      <h2>Editar Usuario #<%=u.getId()%></h2>
      <form method="post" action="modificarUsuarios.jsp?id=<%=u.getId()%>">
        <label>Nombre:</label>
        ...
        <input type="password" name="contraseña" value="<%=u.getContraseña()%>" required/>
        ...
        <select name="rol" required>
          ...
          <%
            for (Rol r : Rol.findAll()) {
              boolean sel = r.getId()==u.getRol().getId();
          %>
...
<%
  if ("POST".equalsIgnoreCase(request.getMethod())) {
    ...
    u.setIdArea((area!=null && !area.isEmpty())?Integer.parseInt(area):null);
    u.saveOrUpdate();
